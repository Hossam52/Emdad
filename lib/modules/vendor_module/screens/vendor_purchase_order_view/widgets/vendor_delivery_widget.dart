...
class VendorDeliveryWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (order.transportationRequest == null) {
      return const _AddTransportationRequest();
...
class _CardWidget extends StatelessWidget {
  const _CardWidget({Key? key, required this.child, this.onEditPrice})
      : super(key: key);
  final Widget child;
  final VoidCallback? onEditPrice;
  @override
...
class _AddTransportationRequest extends StatelessWidget {
  const _AddTransportationRequest({Key? key}) : super(key: key);
              ...
              onPressed: () async {
                await showDialog<bool>(
                  ...
                  builder: (_) => BlocProvider.value(
                    ...
                    child: VendorOrderBlocConsumer(listener: (context, state) {
                      if (state
